...
// cette fonction sera pour l'affichage
app.get('/affiche', function(req, res){
	db.collection('cart', function(err, collection) {
		collection.find().toArray(function(err, i) {
			result = i;
			res.render('affiche', {items : i, error:''} );	//get all data from db and render on index page
		});
	});
});
...
io.sockets.on('connection', function (socket) {	
	...
	app.post('/add', function(req, res){ 
		...
		if(name.match(letters) && price.match(numbers) && qty.match(numbers))
		{
			<!-- add item into db. -->
			db.collection('cart', function(err, collection) {
